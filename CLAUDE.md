...
- **Частичный результат вместо отказа.** Отвалившийся провайдер даёт
  `completeness=partial` с причиной, а не исключение наружу. Пустой ответ —
  тоже ответ: несуществующий прогон едет конвертом с `data: null`, поэтому
  соответствующие MCP-инструменты объявлены как `Envelope[X | None]` (SDK
  валидирует выход по типу).
- **Отказ выполнить запрос — не то же самое, что неполные данные.** Негодный
  аргумент или конфигурация: CLI печатает конверт `empty` и выходит с кодом 1
  (код 1 всегда печатает валидный JSON), MCP поднимает `ToolError` — так SDK
  называет ожидаемый отказ, и только его текст доходит до клиента.
