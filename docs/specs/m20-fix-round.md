...
- **AC-007.** Пять мутантов A–E добавлены в `tests/mutation_gate_demand.py` тем
  ...
  якорь ровно один раз, целевой тест зелёный на чистом дереве). Гейт проходит,
  печатает `MUTATION GATE PASSED` с кодом возврата 0, и ни одна из мутаций,
  бывших на базовом коммите, не удалена — счётчик СТРОГО БОЛЬШЕ базового.
  `bash -c 'cd /home/deploy/exec-clones/gkai-m20-research-demand && .venv/bin/python tests/mutation_gate_demand.py'`

  > Исходно здесь стояло требование напечатать ровно `MUTATION GATE PASSED: 26`,
  > и это была моя арифметическая ошибка: 26 = 21 + 5, где 21 — счётчик гейта ДО
  > вехи M20. На базовом коммите `d4fc960` мутаций уже 27, после добавления пяти
  > стало 32, и числа 26 не могло получиться ни при каком исходе работы.
  > Исполнитель сделал всё верно, гейт прошёл на 32 — и вместо того чтобы
  > выкинуть шесть мутаций ради буквы критерия, честно доложил `blocked` с
  > объяснением. Критерий переписан на проверяемое утверждение: абсолютное число
  > протухает при каждой следующей вехе, дописывающей мутацию, а смысл
  > («обвязку не ослабили») от числа не зависит. Статус `blocked` в
  > `report.json` того прогона остаётся как есть — он верен.
