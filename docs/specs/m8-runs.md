...
- `providers/**`, `pipeline/scenarios.py`, `pipeline/budget.py`,
  ...
  `cli/main.py` и создаются новые файлы из «Что делать». Существующие 243
  теста должны остаться зелёными без правок: если приходится править
  существующий тест — контракт принятой вехи сломан, остановись и доложи.
